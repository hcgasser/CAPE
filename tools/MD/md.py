...
if __name__ == "__main__":
    ...
    argparser.add_argument('--pdb', type=str, help='the pdb file to simulate. if non is provided the output subfolders will be renamed using the current parameter hash')
    argparser.add_argument('--output', type=str, default='.', help='the output folder')
    argparser.add_argument('--solvent', type=str, default='implicit', help='implicit/explicit')
    argparser.add_argument('--temperature', type=float, default=25.0, help='temperature in degrees Celsius')
    argparser.add_argument('--time_span', type=float, default=2000, help='simulation time span in nanoseconds')
    argparser.add_argument('--step_size', type=float, default=2, help='step-size in femtoseconds')
    argparser.add_argument('--freq_report', type=float, default=2000, help='report state every freq_report steps')
    argparser.add_argument('--num_pdbs', type=int, default=10, help='how many structures to write to the output.pdb')
    argparser.add_argument('--rmsd', action='store_true')
    argparser.add_argument('--resume', action='store_true', help='continue the simulation from a checkpoint')
    argparser.add_argument('--pbar', type=int, default=100, help='how many progress bar updates should there be')
    ...
    temperature = args.temperature + 273.15  # converts the degrees Celsius into degrees Kelvin
    freq_report = args.freq_report
    steps = math.ceil((args.time_span*nanoseconds).value_in_unit(femtoseconds)/args.step_size)
    ...
    integrator = LangevinIntegrator(temperature*kelvin, friction, args.step_size*femtoseconds)
    ...
    alpha_carbon_atom_indices = [atom.index for atom in simulation.topology.atoms() if atom.name == 'CA']  # atom.residue.name in aa3]  # remove HOH (water), NA (sodium)
    
    print('add reporters')
    simulation.reporters.append(CheckpointReporter(os.path.join(output_folder, 'checkpoint.ckpt'), int(steps/args.num_pdbs)))
    
    # simulation.reporters.append(DCDReporter(os.path.join(output_folder, 'output_full.dcd'), freq_report, append=continue_simulation))
    simulation.reporters.append(DCDSubReporter(os.path.join(output_folder, 'output_sub.dcd'), freq_report, atom_indices=alpha_carbon_atom_indices, append=continue_simulation))
    ...
    simulation.reporters.append(PDBReporter(os.path.join(output_folder, 'output.pdb'), int(steps/args.num_pdbs)))
    
    reporter = StateDataReporter(os.path.join(output_folder, 'report.tsv'), int(steps/1000), append=continue_simulation,
        step=True, potentialEnergy=True, kineticEnergy=True, totalEnergy=True, temperature=True, volume=True, separator='\t')
    simulation.reporters.append(reporter)
    
    print(f'simulate {steps:,} steps using {simulation.context.getPlatform().getName()} platform')
    ...
    if args.rmsd:
        ...
        rmsd = md.rmsd(target=trajectory, reference=trajectory, frame=0, atom_indices=alpha_carbon_atom_indices)
