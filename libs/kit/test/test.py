...
from kit.data import remove_lines_between, remove_lines
...
def test_remove_lines_between():
    ...
    result = remove_lines_between(lines, "These", "with", remove_start=False, remove_end=False)
    assert result == expected_result

    # Test 2
    expected_result = ["These", "with"]
    result = remove_lines_between(lines, "are", "with", remove_start=True, remove_end=False)
    assert result == expected_result

    # Test 3
    expected_result = ["These", "are", "only", "with"]
    result = remove_lines_between(lines, "only", "test", remove_start=False, remove_end=True)
    assert result == expected_result

    # Test 4
    expected_result = ["These", "are", "only", "test", "with"]
    result = remove_lines_between(lines, "only", "test", remove_start=False, remove_end=False)
