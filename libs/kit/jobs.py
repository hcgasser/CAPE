...
Each training process is referred to as a job. A job has a unique ID that consists of its environment name, and an
...
from kit.data import df_get_last_entry, DD, df_get_highest_in_category, Split
...
def get_job_info(job_id, metrics=False, mhparams=False, dhparams=False, ckpt=None):
    result = {}
    cont_path = os.path.join(G.ENV.JOBS, job_id)
    task_file_path = os.path.join(cont_path, f"TASK.yaml")
    if not os.path.exists(task_file_path):
        raise FileNotFoundError(f"Job '{job_id}' not found.")
    task = DD.from_yaml(task_file_path)
    df = None
    if (metrics or ckpt is not None) and os.path.exists(os.path.join(cont_path, "metrics.csv")):
    ...
    while 'JOB_TO_CONTINUE' in task:
        ...
        if (metrics or ckpt is not None) and os.path.exists(os.path.join(cont_path, "metrics.csv")):
            ...
            df = pd.concat([df_tmp.query(f"step < {df.step.min()}"), df]) if df is not None else df_tmp

        task = DD.from_yaml(os.path.join(cont_path, f"TASK.yaml"))

    if mhparams:
        result['mhparams'] = DD.from_yaml(os.path.join(cont_path, f"MHPARAMS.yaml"))

    if dhparams:
        result['dhparams'] = DD.from_yaml(os.path.join(cont_path, f"DHPARAMS.yaml"))
    ...
    if ckpt:
        ...
        ckpt_path = os.path.join(G.ENV.JOBS, ckpt_job_id, "checkpoints", f"{ckpt}.ckpt")
...
def plot_job_metrics(filename, job_id, x, y, splits=False, xscale='linear', yscale='linear'):
    ...
    if splits:
        ...
        for split in Split:
            ...
            if col in df.columns:
                df_tmp = df[[col, "epoch"]].\
                    query(f"{col}.notna()").rename(columns={col: y})
                df_tmp["split"] = split.long_name()
                if x == "epoch":
                    df_tmp2 = df_get_highest_in_category(df_tmp, "step", "epoch", f"{y}.notna()")
                    df_tmp = df_tmp2.join(df_tmp, on="step", how="left").drop(columns=["epoch"])
