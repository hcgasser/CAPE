...
def check_seq_hashes_in_loch(loch_path=None, predictor_structure_name='AF', repair_filename=False):
        ...
        for file_name in file_names:
            if file_name.endswith('.fasta'):
                ...
                if filename_seq_hash != seq_hash:
                    ...
                    if repair_filename:
                        os.rename(fasta_file_path, os.path.join(dir_path, f"{seq_hash}.fasta"))
                checked += 1
    log_info(f"Checked {checked} fasta files - {len(fasta_mismatches)} mismatches")

    checked = 0
    for dir_path, _, file_names in os.walk(get_pdb_file_path(loch_path=loch_path, predictor_structure_name=predictor_structure_name)):
        for file_name in file_names:
            if file_name.endswith('.pdb'):
                ...
                if filename_seq_hash != seq_hash:
                    ...
                    if repair_filename:
                        os.rename(pdb_file_path, os.path.join(dir_path, f"{seq_hash}_{predictor_structure_name}.pdb"))
                checked += 1
