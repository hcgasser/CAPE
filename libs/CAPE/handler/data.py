...
from kit.loch import get_fasta_file_path
...
def run():
    if G.TASK.ID == 'data_raw_to_input':
        ...
        for split in ['TRAIN', 'VAL', 'TEST']:
            seq_hashes_file_path = os.path.join(G.PROJECT_ENV.ARTEFACTS, f'{G.DOMAIN}.data.{split}.seq_hash') 
            print(split)
            filename = kit.path.join(G.DHPARAMS.INPUT_FOLDER, f'{split}.fasta')
            df = read_fasta(filename, return_df=True, evaluate=True)
        
            seq_hashes = []
            for seq, row in df.iterrows():
                seq_hash = get_seq_hash(seq)
                seqs_to_fasta(seq, get_fasta_file_path(seq_hash), seq_hash, translate=('','','*-'))
    ...
    if G.TASK.ID == 'create_support':
        ...
        dfs = []
        for split in kit.Split:
            if str(split) in G.DHPARAMS:
                dfs.append(kit.bioinf.fasta.read_fasta(os.path.join(G.ENV.INPUT, G.DHPARAMS[str(split)]),
                                                       evaluate=True,
                                                       return_df=True))
        ...
        supports_seq_fasta_file_path = os.path.join(G.PROJECT_ENV.ARTEFACTS, f"{G.DOMAIN}.support.fasta")
        kit.bioinf.fasta.write_fasta(supports_seq_fasta_file_path, df_support)

        supports_seq_hashes_file_path = os.path.join(G.PROJECT_ENV.ARTEFACTS, f"{G.DOMAIN}.support.seq_hash")        
